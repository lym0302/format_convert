...
PARSER.add_argument('-e',
                    "--end",
                    default = False,
                    help = "When the last slice is shorter than sample_slice_length_ms, save ot not? False is not True is save")

ARGS = PARSER.parse_args()

#Function to slice up the audio.
#def slice_audio(file, channels, outformat, width, rate, slice_length, slide):
def slice_audio(file, channels, outformat, width, rate, slice_length, slide, end):
...
#	for file in files:
	if True:
		...
		#notes_reversed.export(ARGS.output_dir+fileName.split('/')[-1]+'notes_reversed_granular.'+outformat, format=outformat)  # a total file
...
def main():
	...
	if ARGS.input_dir:
		for file in os.listdir(ARGS.input_dir):
			...
			slice_audio(ARGS.input_dir+file, ARGS.channels, ARGS.out_format, ARGS.sample_width, ARGS.sample_rate, ARGS.sample_slice_length_ms, ARGS.window_slide_ms, ARGS.end)
		...
		slice_audio(ARGS.infiles, ARGS.channels, ARGS.out_format, ARGS.sample_width, ARGS.sample_rate, ARGS.sample_slice_length_ms, ARGS.window_slide_ms, ARGS.end)
main()
